...
The MCP tool names written throughout this skill (`mcp__claude_ai_Consensus__search`, `mcp__claude_ai_PubMed__search_articles`, `mcp__claude_ai_bioRxiv__search_preprints`, `mcp__claude_ai_Scite__search_literature`, and the `mcp__tavily__*` family) are **illustrative Claude-Desktop names**. The registered names are **host-dependent**: some environments expose these servers under opaque IDs (e.g. `mcp__<uuid>__search`) instead of the `claude_ai_*` names, so a literal call to a hardcoded name can fail with "tool not found."
